...
| `notification` | 子需要用户注意（AskUserQuestion / 超时提示 / idle 长等） | 先 capture-pane 看原因，别盲目响应 |
| `stop` | 子空闲 | 发下条指令 |
| `stop_failure` | 子异常终止 | 介入排查 |

**Notification 的语义 warning**：Claude Code 的 Notification hook 触发条件 documented 不详，实测包括：AskUserQuestion 菜单、60s+ idle、需要响应的提示对话。父 agent 收到 `notification` signal 先用 `psmux capture-pane -t <session> -p | tail -10` 看原因，**不要**盲目当 stop 处理（会 double-prompt）。在父 agent prompt 里硬约束子"不用 AskUserQuestion"可减少此类 signal。
